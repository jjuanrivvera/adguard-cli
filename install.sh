#!/bin/sh
# adguard-home installer for macOS and Linux.
#
...
#   ADGUARD_HOME_VERSION=v1.2.3   pin a version (default: the latest release)
...
REPO="jjuanrivvera/adguard-cli"
BINARY="adguard-home"
VERSION="${ADGUARD_HOME_VERSION:-}"
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "could not determine the latest release; set ADGUARD_HOME_VERSION"
fi
